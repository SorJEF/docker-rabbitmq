ENVS = --dns 8.8.8.8  -e DNSSERVER=$${DNS_SERVER:-'8.8.8.8'} -e RABBITMQ_PASSWORD=$${RABBITMQ_PASSWORD} -e LDAP_SERVER=$${LDAP_SERVER:-'ldap.wot.io'} -e VHOSTS=$${VHOSTS:-'wot'} -h $$(hostname -s)
...
CONTAINER = rabbitmq
CLUSTER ?=
VOLUMES = -v `pwd`/data:/var/lib/rabbitmq/mnesia
